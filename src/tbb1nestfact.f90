...
! The subroutine lt1derivs is in file t12fact.f90
! The subroutine lbb1derivs is in file bb1facts.f90

! nested-factor model with t linking copulas for group to common latent
...
!   tl = vector of transformed quadrature nodes  (via qt(u,nu))
! outputs 
!   nllk = negative log-likelihood, 
!   grad = gradient of nllk, 
!   hess = hessian of nllk
subroutine strtbb1(npar,th,mgrp,n,dvar,nu,grsize,udata,nq,wl,xl,tl,nllk,grad,hess)
  implicit none
  integer npar,mgrp,dvar,n,nq,ip,jp, ind
  double precision nu,th(npar),udata(n,dvar),wl(nq),xl(nq),tl(nq),uvec(dvar)
    ...
    do iq =1,nq
      ...
      do jg =1,mgrp   !jth group
        ...
        do iq2 =1,nq   
          ...
          call lt1derivs(tl(iq2),tl(iq),th(jg2),nu,lpdf(jg2),der1(jg2),der2(jg2))    !c_{V_j,V_0}  
