...
! The subroutine gum1derivs is in file gum12fact.f90

! nested-factor model with Gumbel linking copulas
! inputs
!   npar = mgrp+dvar = number of parameters
!    same order as simnestfact() function in R
...
subroutine strgum1(npar,th,mgrp,n,dvar,grsize,udata,nq,wl,xl,nllk,grad,hess)
  implicit none
  ...
  nllk=0.d0; grad = 0.d0; hess=0.d0 !npar = dvar+mgrp; dvar = sum(grsize)
  do i =1,n 
    uvec = udata(i,:) 
    ...
    do iq =1,nq
      ...
      do jg =1,mgrp   !jth group                         
        ...
        do iq2 =1,nq   
          ...
          call lgum1derivs(xl(iq2),xl(iq),th(jg2),lpdf(jg2),der1(jg2),der2(jg2))   !c_{V_j,V_0}  
