...
!  end do
!  print *, "1-factor"
!  call irgau1finfo(npar,param,d,ncat,ucuts,nq,wl,xl,finfo)
!  print "(8e13.4/)", finfo
...
!  end do
!  print *, "2-factor"
!  call irgau2finfo(npar,param,d,ncat,ucuts,nq,wl,xl,finfo)
!  print "(8e13.4/)", finfo
!  deallocate (xl, wl, param, finfo, ucuts)
!  stop
!  end

! Subroutines irgau1derivs irgau2derivs are in file irgau12fact.f90
...
subroutine irgau1finfo(npar,th,d,ncat,ucuts,nq,wl,xl,finfo)
  implicit none
...
! inputs 
!   npar = #parameters,
!   th = parameter vector (dimension 2d=npar), th[j]>1
!   d = #items
!   ncat = #categories (ordinal)
!   ucuts = (ncat-1)xd matrix of cutpoints on Uniform(0,1) scale
!   nq = #quadrature points
...
! output:
!   finfo = Fisher information matrix
subroutine irgau2finfo(npar,param,d,ncat,ucuts,nq,wl,xl,finfo)
  implicit none
  integer npar,d,ncat,nq
