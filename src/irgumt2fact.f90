! Gumbel/t 2-factor copula models for discrete item response data

! gcc -c ptbeta.c
! gfortran -o irgumt12fact irgumt12fact.f90 irgum12fact.f90  irt12fact.f90 qtnorm.f90 ptbeta.o
...
!  end do
!  print *, param
!  print *, "2-factor Gumbel/t"
!  call irgumt2fact(npar,param,nu2,d,ncat,n,ydata,ucuts,nq,wl,xl,nllk,grad,hess)
!  print *, nllk
!  print "(8f10.6)", grad
!  print "(8f10.6/)", hess
...
! 2-factor model with Gumbel/t
! Subroutine irgum1derivs is in file irgum12fact.f90
! Subroutine irt2derivs is in file irt12fact.f90
! inputs 
!   npar = #parameters,
...
subroutine irgumt2fact(npar,param,nu2,d,ncat,n,ydata,ucuts,nq,wl,xl,nllk,grad,hess)
  implicit none
  ...
  allocate ( tl2(nq), tcuts(ncat-1,d) )
  ...
  do iq=1,nq ! convert to t(nu2) scale
    tl2(iq)=qt(xl(iq),nu2)
  end do
