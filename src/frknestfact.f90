...
! nested-factor model with Frank linking copulas
! inputs
...
subroutine strfrk1(npar,th,mgrp,n,dvar,grsize,udata,nq,wl,xl,nllk,grad,hess)
  implicit none
      ...
      do jg =1,mgrp   !jth group                         
        ...
        do iq2 =1,nq   
          ...
          do mj = ind1,ind2 ! index for within subgroup
            nj=mj+mgrp
            call lfrk1derivs(uvec(mj),xl(iq2),th(nj),lpdf(nj),der1(nj),der2(nj))   !c_{ij,V_j}   
            lk = lk*exp(lpdf(nj))
          end do
