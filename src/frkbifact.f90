...
! Subroutines lfrk1derivs lfrk2derivs clfrkderivs are in file frk12fact-lpdf.f90
! Subroutines lfrkpdf cfrkcdf are in file cop12factlik.f90

! bi-factor model with Frank linking copulas
! inputs
...
subroutine strfrk2(npar,th,mgrp,n,dvar,grsize,udata,nq,wl,xl,nllk,grad,hess)
  implicit none
      ...
      do jg =1,mgrp   !jth group                         
        ...
        do iq2 =1,nq   
          ...
          do mj = ind1,ind2  ! within group index
            call clfrkderivs(uvec(mj),xl(iq),th(mj),ccdf(mj),cder1(mj),cder2(mj))  !C_{ij|V_0} 
            ...
            call lfrk1derivs(uvec(mj),xl(iq),th(mj),lpdf(mj),der1(mj),der2(mj)) !c_{ij,V_0}  
            lk = lk*exp(lpdf(mj)) 
...
! output 
!   nllk = negative log-likelihood
subroutine strfrk2nllk(npar,th,mgrp,n,dvar,grsize,udata,nq,wl,xl,nllk)
  implicit none
    ...
    do iq =1,nq
      ...
      do jg =1,mgrp   !jth group                         
        ...
        do iq2 =1,nq   
          lk = 1.d0; lk2 = 1.d0   
          do mj = ind1,ind2 ! within group index
            call cfrkcdf(uvec(mj),xl(iq),th(mj),ccdf(mj)) !C_{ij|V_0}
            if(ccdf(mj) < 0.00001) ccdf(mj) = 0.00001
            if(ccdf(mj) > 0.99999) ccdf(mj) = 0.99999    
            call lfrkpdf(ccdf(mj),xl(iq2),th(dvar+mj),lpdf(dvar+mj)) !c_{i,V_j;V_0}  
            call lfrkpdf(uvec(mj),xl(iq),th(mj),lpdf(mj)) !c_{ij,V_0}  
            lk = lk*exp(lpdf(mj))
            lk2 = lk2*exp(lpdf(dvar+mj))
          end do
          llk = lk*lk2
