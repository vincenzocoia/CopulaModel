...
! Subroutines lgum2derivs is in file gum12factf.f90
! Subroutines lbb1pdf lgumpdf cbb1cdf are in file cop12factlik.f90

! bi-factor model with BB1 linking copulas for common latent
!                 and Gumbel linking copula for group latent
! inputs
...
subroutine strbb1gum2(npar,th,mgrp,n,dvar,grsize,udata,nq,wl,xl,nllk,grad,hess)
  implicit none
      ...
      do jg =1,mgrp   !jth group                         
        ...
        do iq2 =1,nq   
          ...
          do mj = ind1,ind2
            ...
            !c_{i,V_j;V_0} 
            call lgum2derivs(ccdf(mj),xl(iq2),th(2*dvar+mj),lpdf(dvar+mj),der1(2*dvar+mj),der2(3*dvar+mj),&
...
! output 
!   nllk = negative log-likelihood 
subroutine strbb1gum2nllk(npar,th,mgrp,n,dvar,grsize,udata,nq,wl,xl,nllk)
  implicit none
      ...
      do jg =1,mgrp   !jth group                         
        ...
        do iq2 =1,nq   
          ...
          do mj = ind1,ind2
            ...
            if(ccdf(mj) > 0.99999) ccdf(mj) = 0.99999 
            !c_{i,V_j;V_0} 
            call lgumpdf(ccdf(mj),xl(iq2),th(2*dvar+mj),lpdf(dvar+mj))
            !c_{ij,V_0}
