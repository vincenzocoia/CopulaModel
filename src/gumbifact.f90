! Subroutines lgum1derivs lgum2derivs cgumderivs are in file gum12fact.f90
! Subroutines lgumpdf cgumcdf are in file cop12factlik.f90

! bi-factor model with Gumbel linking copulas
! inputs
...
subroutine strgum2(npar,th,mgrp,n,dvar,grsize,udata,nq,wl,xl,nllk,grad,hess)
  implicit none
      ...
      do jg =1,mgrp   !jth group                         
        ...
        do iq2 =1,nq   
          ...
          do mj = ind1,ind2 ! within group index
            ...
            call lgum2derivs(ccdf(mj),xl(iq2),th(dvar+mj),lpdf(dvar+mj),  &
...
! output 
!   nllk = negative log-likelihood
subroutine strgum2nllk(npar,th,mgrp,n,dvar,grsize,udata,nq,wl,xl,nllk)
  implicit none
      ...
      do jg =1,mgrp   !jth group
        ...
        do iq2 =1,nq  
          ...
          do mj = ind1,ind2 ! within group index
            ...
            if(ccdf(mj) < 0.00001) ccdf(mj) = 0.00001
            if(ccdf(mj) > 0.99999) ccdf(mj) = 0.99999 
            call lgumpdf(ccdf(mj),xl(iq2),th(dvar+mj),lpdf(dvar+mj))  !c_{i,V_j;V_0}  
